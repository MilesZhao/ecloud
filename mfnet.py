...
def alexnet_a(inputs, dim):
    x = inception_a(inputs,20)
    x = inception_a(x,20)
    x = inception_a(x,20)
...
def inception_b(x,dim=10):
    b0 = tf.keras.layers.Conv2D(dim,kernel_size=(1,1),strides=(1,1),
        padding='same',activation='relu')(x)

    b1 = tf.keras.layers.Conv2D(dim,kernel_size=(1,1),strides=(1,1),
        padding='same',activation='relu')(x)
    b1 = tf.keras.layers.Conv2D(dim,kernel_size=(7,1),strides=(1,1),
        padding='same',activation='relu')(b1)
    b1 = tf.keras.layers.Conv2D(dim,kernel_size=(1,7),strides=(1,1),
        padding='same',activation='relu')(b1)

    b2 = tf.keras.layers.Conv2D(dim,kernel_size=(1,1),strides=(1,1),
        padding='same',activation='relu')(x)
    b2 = tf.keras.layers.Conv2D(dim,kernel_size=(7,1),strides=(1,1),
        padding='same',activation='relu')(b2)
    b2 = tf.keras.layers.Conv2D(dim,kernel_size=(1,7),strides=(1,1),
        padding='same',activation='relu')(b2)
    b2 = tf.keras.layers.Conv2D(dim,kernel_size=(7,1),strides=(1,1),
        padding='same',activation='relu')(b2)
    b2 = tf.keras.layers.Conv2D(dim,kernel_size=(1,7),strides=(1,1),
...
def alexnet_b(inputs, dim):
    x = inception_b(inputs,20)
    x = inception_b(x,20)
    x = inception_b(x,20)
...
def inception_c(x,dim=10):
    b0 = tf.keras.layers.Conv2D(dim,kernel_size=(1,1),strides=(1,1),
        padding='same',activation='relu')(x)

    b1 = tf.keras.layers.Conv2D(dim,kernel_size=(1,1),strides=(1,1),
        padding='same',activation='relu')(x)
    b10 = tf.keras.layers.Conv2D(dim,kernel_size=(3,1),strides=(1,1),
        padding='same',activation='relu')(b1)
    b11 = tf.keras.layers.Conv2D(dim,kernel_size=(1,3),strides=(1,1),
        padding='same',activation='relu')(b1)
    b1 = tf.keras.layers.Concatenate(axis=-1)([b10,b11])

    b2 = tf.keras.layers.Conv2D(dim,kernel_size=(1,1),strides=(1,1),
        padding='same',activation='relu')(x)
    b2 = tf.keras.layers.Conv2D(dim,kernel_size=(3,1),strides=(1,1),
        padding='same',activation='relu')(b2)
    b2 = tf.keras.layers.Conv2D(dim,kernel_size=(1,3),strides=(1,1),
        padding='same',activation='relu')(b2)
    b20 = tf.keras.layers.Conv2D(dim,kernel_size=(1,3),strides=(1,1),
        padding='same',activation='relu')(b2)
    b21 = tf.keras.layers.Conv2D(dim,kernel_size=(3,1),strides=(1,1),
        padding='same',activation='relu')(b2)
    b2 = tf.keras.layers.Concatenate(axis=-1)([b20,b21])
...
def alexnet_c(inputs, dim):
    x = inception_c(inputs,20)
    x = inception_c(x,20)
    x = inception_c(x,20)
...
def inception(x,dim=10):
    conv1 = tf.keras.layers.Conv2D(dim,kernel_size=(1,1),strides=(1,1),\
        padding='same',activation='relu')(x)
    conv2 = tf.keras.layers.Conv2D(dim,kernel_size=(3,3),strides=(1,1),\
        padding='same',activation='relu')(x)
    conv3 = tf.keras.layers.Conv2D(dim,kernel_size=(5,5),strides=(1,1),\
        padding='same',activation='relu')(x)
    conv4 = tf.keras.layers.Conv2D(dim,kernel_size=(7,7),strides=(1,1),\
        padding='same',activation='relu')(x)
    pool = tf.keras.layers.MaxPooling2D(pool_size=(5,5),strides=(1,1),padding='same')(conv1)
    conv = tf.keras.layers.Concatenate(axis=-1)([conv1, conv2, conv3, conv4, pool])
    pool = tf.keras.layers.MaxPooling2D()(conv)
    pool = tf.keras.layers.BatchNormalization()(pool)
    return pool

def alexnet(inputs, dim):
    x = inception(inputs,20)
    x = inception(x,20)
    x = inception(x,20)
...
def normal_net2d(dim = 24):
    ...
    h_xzy = alexnet_a(xzy_view,dim=dim)
    ...
    h_zyx = alexnet_b(zyx_view,dim=dim)
    ...
    h_yxz = alexnet_c(yxz_view,dim=dim)
