...
def senet_module(inputs,inter_dim):
    ...
    excitation = Dense(inter_dim//4,kernel_initializer=init, activation='relu')(squeeze)
...
def inception(x,dim=128):
    init = tf.keras.initializers.glorot_uniform(123)
    conv1 = tf.keras.layers.Conv2D(dim,kernel_size=(1,1),strides=(1,1),\
        padding='same',activation='relu',kernel_initializer=init)(x)
...
def normal_net2d(dim = 24):
    ...
    x = Dense(256,kernel_initializer=init,activation='relu',kernel_regularizer=kr)(x)
